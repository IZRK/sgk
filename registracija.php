...
function isWithinScheduleWindow($window, $date = null) {
...
function isRegistrationOptionAvailable($option, $schedule, $optionPeriods) {
...
function isPresentationSelectionAvailable($presentationType, $schedule) {
...
function isRegistrationSubmissionOpen($schedule) {
	return isWithinScheduleWindow($schedule['registration_submission'] ?? [], $schedule['today'] ?? null);
}

function registrationEditSalt() {
	return trim((string)(getenv('SGK_EDIT_SALT') ?: getenv('SGK_ADMIN_SALT') ?: ''));
}

function buildSubmissionEditKey($submittedAt, $email) {
	return hash('md5', mb_strtolower(trim($email), 'UTF-8') . '|' . trim($submittedAt) . '|' . registrationEditSalt());
}

function buildSubmissionEditUrl($submittedAt, $email) {
...
function readSubmissionTable($path) {
	if (!is_file($path)) {
		return [
			'headers' => [],
			'rows'    => [],
...
function findSubmissionByEditKey($csvPath, $editKey) {
...
function updateSubmissionInCsv($csvPath, $submittedAt, $email, $updates) {
	$table = readSubmissionTable($csvPath);
	if ($table['error'] !== null) {
		return false;
	}
	...
	foreach ($table['rows'] as $index => $row) {
		if (trim((string)($row['submitted_at'] ?? '')) !== $submittedAt ||
...
function saveSubmissionToCsv($csvPath, $row) {
	$dir = dirname($csvPath);
	if (!is_dir($dir) && !mkdir($dir, 0755, true) && !is_dir($dir)) {
		return false;
	}
...
function deriveInvoiceFields($formData) {
	$fullName = trim($formData['first_name'] . ' ' . $formData['last_name']);
	$invoiceName = trim($formData['institution']) !== '' ? trim($formData['institution']): $fullName;
...
$availableRegistrationTypes = array_filter(array_keys($registrationPrices),
	static fn($option) => isRegistrationOptionAvailable($option, $registrationSchedule, $registrationOptionPeriods));
$registrationSubmissionOpen = isRegistrationSubmissionOpen($registrationSchedule);
$abstractSubmissionOpen =
	isWithinScheduleWindow($registrationSchedule['abstract_submission'], $registrationSchedule['today']);
...
if ($editKey !== '') {
	if (registrationEditSalt() === '') {
		...
		if ($editTableMatch === null) {
			...
			$editRequiresAbstract = in_array($formData['presentation_type'], [
				'Predavanje',
				'Plakat'
			], true);
...
if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
	...
	$requiresAbstract = in_array($formData['presentation_type'], [
		'Predavanje',
		'Plakat'
	], true);
	$abstractWordCount = countAbstractWords($formData['abstract_text']);
	...
	if (empty($errors)) {
		...
		$message = $isEditMode ? '<p>Posodobitev prijave na 7. SGK.</p>': '<p>Nova prijava na 7. SGK.</p>';
		...
		if ($qrImageCid !== null) {
			...
			$message .= '<p style="margin:0;text-align:center;"><img src="cid:' . e($qrImageCid) .
				'" alt="UPN QR za plačilo 7. SGK" style="display:inline-block;width:250px;max-width:100%;height:auto;background:#ffffff;"></p>';
		}

		if ($editUrl !== null) {
			$message .= '<p>Naslov prispevka in povzetek lahko urejate do roka za oddajo povzetkov na povezavi: <a href="' .
				e($editUrl) . '">' . e($editUrl) . '</a></p>';
		}
		...
		if (empty($errors)) {
			$subject = $isEditMode ? '7. SGK popravek prijave - ' . $editSubjectName:
...
$showAbstractFields = in_array($formData['presentation_type'], [
	'Predavanje',
	'Plakat'
], true);
		...
		<?php else : ?>
            <article class="panel">
                ...
                <table class="fees-table">
                    <thead>
                    <tr>
                        <th>Vrsta kotizacije</th>
                        <th>Cena (EUR) z DDV</th>
                        <th>Opomba</th>
                    </tr>
                    </thead>
                    <tbody>
                    ...
                    <tr>
                        ...
                        <td>300,00</td>
                        <td>* članarina za 2026: <a href="https://www.slovenskogeoloskodrustvo.si/clanstvo/"
                ...
                <p>Kotizacija vključuje: kongresni paket s tiskanim zbornikom in vodnikom, okrepčila med odmori, kosilo
                    prvi in drugi dan, kongresno večerjo prvi dan, zaključno večerjo drugi dan.</p>
                <p>Stroški namestitve niso vključeni v kotizacijo. Rezervacija in plačilo namestitve potekata preko
                    povezave: <a href="https://www.lipica.org/sl/prijava-na-kongres/" target="_blank" rel="noreferrer">https://www.lipica.org/sl/prijava-na-kongres/</a>.
                    Z geslom <strong>lipica1580</strong> dostopate do kongresnih cen namestitve. Povezava bo na voljo
                    tudi ob prijavi, na spletni strani kongresa. Kongresne cene za namestitve so namenjene udeležencem
                    kongresa.</p>
            </article>
			...
			<?php if ($isEditMode) : ?>
                <article class="panel">
					...
					<?php if ($isEditAllowed) : ?>
                        <p>Spreminjate lahko samo naslov prispevka in povzetek. Ostali podatki so prikazani samo
                            informativno.</p>
			...
			<?php elseif ($formLocked) : ?>
                <article class="panel">
					<h3>REGISTRACIJE SO ZAKLJUČENE</h3>
					<p>Oddaja registracij in povzetkov je zaključena.</p>
                </article>
			<?php endif; ?>
			...
			<?php if (!$formLocked) : ?>
                <form method="post" class="registration-form" id="registration-form">
                    ...
                    <fieldset style="border:0;padding:0;margin:0;" <?= $isEditMode ? 'disabled': '' ?>>
                        ...
                        <label class="inline-check"><input type="checkbox" name="invoice_same"
                                                           value="1" <?= $formData['invoice_same'] === '1' ? 'checked':
								'' ?>> Enako kot zgoraj</label>
                        <div class="form-grid">
                            ...
                            <label>Naslov
                                <input type="text" name="invoice_address"
                                       value="<?= e($formData['invoice_address']) ?>">
                            </label>
                            <label>Pošta
                                <input type="text" name="invoice_post" value="<?= e($formData['invoice_post']) ?>">
                            </label>
                            <label>Država
                                <input type="text" name="invoice_country"
                                       value="<?= e($formData['invoice_country']) ?>">
                            </label>
                        </div>
                        <div class="form-grid">
                            <label class="inline-check"><input type="checkbox" name="vat_payer"
                                                               value="1" <?= $formData['vat_payer'] === '1' ? 'checked':
									'' ?>> Davčni zavezanec</label>
                            <label>ID za DDV
                        ...
                        <div class="form-grid">
                            <label>Vrsta kotizacije*
                                <select name="registration_type" id="registration_type" required>
									...
									<?php foreach ($registrationTypeOptions as $value => $label) : ?>
										<?php if (!in_array($value, $availableRegistrationTypes, true)) {
											continue;
										} ?>
                                        <option
                                                value="<?= e($value) ?>"
                            ...
                            <label>Način plačila*
                                <select name="payment_method" required>
                                    <option value="">Izberite</option>
                                    <option value="bančno nakazilo" <?= $formData['payment_method'] ===
									'bančno nakazilo' ? 'selected': '' ?>>Bančno nakazilo
                                    </option>
                                    <option value="naročilnica - plačilo po računu" <?= $formData['payment_method'] ===
									'naročilnica - plačilo po računu' ? 'selected': '' ?>>Naročilnica - plačilo po
                                        računu
                                    </option>
                                    <option value="drugo" <?= $formData['payment_method'] === 'drugo' ? 'selected':
										'' ?>>Drugo
                                    </option>
                                </select>
                            </label>
                        ...
                        <fieldset>
                            ...
                            <label class="inline-check"><input type="radio" name="mid_excursion" value="none"
                                                               data-price="0" <?= $formData['mid_excursion'] ===
								'none' ? 'checked': '' ?>> Brez izbire (0,00)</label>
                            <label class="inline-check"><input type="radio" name="mid_excursion" value="kobilarna"
                                                               data-price="16" <?= $formData['mid_excursion'] ===
								'kobilarna' ? 'checked': '' ?>> Ogled kobilarne Lipica (16,00)</label>
                            <label class="inline-check"><input type="radio" name="mid_excursion" value="kamnolom"
                                                               data-price="0" <?= $formData['mid_excursion'] ===
								'kamnolom' ? 'checked': '' ?>> Ogled kamnoloma Lipica (0,00)</label>
                        </fieldset>

                        <div class="form-grid">
                            <label class="inline-check"><input type="checkbox" name="post_excursion" value="1"
                                                               data-price="40" <?= $formData['post_excursion'] === '1' ?
									'checked': '' ?>> Pokongresna ekskurzija (3. 10. 2026) - 40,00</label>
                            <label class="inline-check"><input type="checkbox" name="photo_contest" value="1"
                                                               data-price="0" <?= $formData['photo_contest'] === '1' ?
									'checked': '' ?> <?= $photoContestDisabledAttr ?>> Sodelovanje na fotografskem
                                natečaju (0,00)</label>
                        </div>
                        ...
                        <div class="form-grid">
                            <label>Model
                                <select name="shirt_gender">
                                    <option value="">Izberite</option>
                                    <option value="Ženska" <?= $formData['shirt_gender'] === 'Ženska' ? 'selected':
										'' ?>>Ženska
                                    </option>
                                    <option value="Moška" <?= $formData['shirt_gender'] === 'Moška' ? 'selected': '' ?>>
                                        Moška
                                    </option>
                                </select>
                            </label>
                            <label>Velikost
                                <select name="shirt_size">
                                    <option value="">Izberite</option>
                                    <option value="XS" <?= $formData['shirt_size'] === 'XS' ? 'selected': '' ?>>XS
                                    </option>
                                    ...
                                    <option value="XL" <?= $formData['shirt_size'] === 'XL' ? 'selected': '' ?>>XL
                                    </option>
                                    <option value="XXL" <?= $formData['shirt_size'] === 'XXL' ? 'selected': '' ?>>XXL
                                    </option>
                                </select>
                            </label>
                        </div>
                        ...
                        <div class="form-grid">
                            <label class="inline-check"><input type="checkbox" name="diet_vegetarian"
                                                               value="1" <?= $formData['diet_vegetarian'] === '1' ?
									'checked': '' ?>> Vegetarijanec</label>
                            <label class="inline-check"><input type="checkbox" name="diet_lactose"
                                                               value="1" <?= $formData['diet_lactose'] === '1' ?
									'checked': '' ?>> Alergija na laktozo</label>
                            <label class="inline-check"><input type="checkbox" name="diet_gluten"
                                                               value="1" <?= $formData['diet_gluten'] === '1' ?
									'checked': '' ?>> Alergija na gluten</label>
                            <label class="inline-check"><input type="checkbox" name="diet_none"
                                                               value="1" <?= $formData['diet_none'] === '1' ? 'checked':
									'' ?>> Nič od naštetega</label>
                        </div>
                        <label>Drugo
                        ...
                        <h3>Predstavitev*</h3>
                        <label class="inline-check"><input type="radio" name="presentation_type"
                                                           value="Predavanje" <?= $formData['presentation_type'] ===
							'Predavanje' ? 'checked': '' ?> <?= $presentationDisabledAttr ?> required>
                            Predavanje</label>
                        <label class="inline-check"><input type="radio" name="presentation_type"
                                                           value="Plakat" <?= $formData['presentation_type'] ===
							'Plakat' ? 'checked': '' ?> <?= $presentationDisabledAttr ?>> Plakat</label>
                        <label class="inline-check"><input type="radio" name="presentation_type"
                                                           value="Brez predstavitve" <?= $formData['presentation_type'] ===
							'Brez predstavitve' ? 'checked': '' ?>> Brez predstavitve</label>
                    </fieldset>
                    ...
                    <label>Opombe
                        <textarea name="notes"
                                  rows="4" <?= $notesReadonlyAttr ?>><?= e($formData['notes']) ?></textarea>
                    </label>

                    <input type="hidden" name="upn_qr_image" id="upn_qr_image"
                           value="<?= e($formData['upn_qr_image']) ?>">
					...
					<?php if (!$isEditMode) : ?>
                        <article class="panel payment-qr-panel" id="payment-qr-panel" hidden>
                            <div class="payment-qr-copy">
                                ...
                                <p>QR koda za nakazilo je na voljo takoj, ko se izračuna znesek prijave. Ista slika se
                                    ob oddaji obrazca priloži tudi v e-mail prijave.</p>
					...
					<?php if ($turnstileConfigured && $showSubmitControls) : ?>
                        <div class="turnstile-wrap">
                            <div class="cf-turnstile" data-sitekey="<?= e($turnstileSiteKey) ?>"
                                 data-theme="light"></div>
                        </div>
					<?php endif; ?>

					<?php if ($showSubmitControls) : ?>
                        <button type="submit" class="btn btn-primary"><?= $isEditMode ? 'Shrani spremembe':
								'Pošlji prijavo' ?></button>
					<?php endif; ?>
                </form>
			<?php endif; ?>
            ...
            <article class="panel">
                ...
                <p>Plačilo kotizacije mora biti izvršeno v roku 30 dni. Udeleženci prejmejo predračun na elektronski
                    naslov.</p>
                <p>Rezervacije nočitev: <a href="https://www.lipica.org/sl/prijava-na-kongres/" target="_blank"
                                           rel="noreferrer">https://www.lipica.org/sl/prijava-na-kongres/</a> (geslo:
                    <strong>lipica1580</strong>)</p>
            </article>
		<?php endif; ?>
    </div>
</section>
